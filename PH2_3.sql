...
BEGIN
    ...
    END;
    BEGIN
        SELECT CHUYENKHOA INTO V_KHOA FROM NHANVIEN WHERE MA_NV = V_USER;
        DBMS_SESSION.SET_CONTEXT('DNX', 'USER_KHOA', V_KHOA);
        EXCEPTION
        WHEN NO_DATA_FOUND THEN
            DBMS_SESSION.SET_CONTEXT('DNX', 'USER_KHOA', 'NONE');
     END;
...
RETURN VARCHAR2;
FUNCTION SELECT_HSBA(OWNER VARCHAR2, OBJNAME VARCHAR2)
RETURN VARCHAR2;
...
BEGIN
    L_CSYT := SYS_CONTEXT('DNX', 'USER_CSYT');
    L_KHOA := SYS_CONTEXT('DNX', 'USER_KHOA');
    IF USER = 'DNX' OR SYS_CONTEXT('DNX', 'USER_VAITRO') = 'Thanh tra' THEN
        PREDICATE := NULL;
    ELSIF  SYS_CONTEXT('DNX', 'USER_VAITRO') = 'Cơ sở y tế' OR SYS_CONTEXT('DNX', 'USER_VAITRO') = 'Y sĩ/Bác sĩ' THEN
        PREDICATE := 'MA_CSYT = ''' || L_CSYT ||'''';
     ELSIF  SYS_CONTEXT('DNX', 'USER_VAITRO') = 'Nghiên cứu' THEN
        PREDICATE := 'MA_CSYT = ''' || L_CSYT ||''' AND MA_KHOA =''' || L_KHOA ||'''';
    ELSE
        PREDICATE := '1=0';
    END IF;
    RETURN PREDICATE;
...
BEGIN
DBMS_RLS.DROP_POLICY('DNX', 'HSBA', 'SELECT_HSBA_POLICY');
END;
...
BEGIN
DBMS_RLS.ADD_POLICY('DNX', 'HSBA', 'SELECT_HSBA_POLICY', 'DNX', 'SECURITY_PACKAGE.SELECT_HSBA', 'SELECT');
END;
...
CREATE OR REPLACE VIEW SELECT_HSBA_DV
AS
SELECT DNX.HSBA_DV.* FROM DNX.HSBA_DV, DNX.HSBA WHERE DNX.HSBA.MA_HSBA = DNX.HSBA_DV.MA_HSBA;

GRANT SELECT ON SELECT_HSBA_DV TO NGHIENCUU;
...
--
--SELECT*FROM DNX.hsba WHERE MA_HSBA = 'HS00001';
--
...
select SYS_CONTEXT('DNX', 'USER_CSYT')from dual;
...
GRANT SELECT ON DNX.HSBA_DV TO CSYT;

GRANT SELECT ON DNX.HSBA TO YBACSI;
GRANT SELECT ON DNX.HSBA_DV TO YBACSI;

GRANT SELECT ON DNX.HSBA TO NGHIENCUU;
GRANT SELECT ON DNX.HSBA_DV TO NGHIENCUU;
